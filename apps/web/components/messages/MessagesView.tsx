...
export default function MessagesView({ portal = "freelancer" }: MessagesViewProps) {
  ...
  const profileLink = activeConv
    ? portal === "client"
      ? otherParticipant(activeConv)?.userId
        ? `/client/freelancers/${otherParticipant(activeConv)?.userId}`
        : `/client/freelancers`
      : `/client/${conversationLabel(activeConv).toLowerCase().replace(/[^a-z0-9]/g, "")}`
    : "#";
    ...
    <main className="flex-1 w-full mx-auto px-6 py-8 flex flex-col h-[calc(100vh-80px)]">
      <div className="flex flex-col mb-6">
        ...
        <p className="text-muted text-sm">
          {portal === "client"
            ? "Communicate with talent, negotiate milestones, and coordinate project delivery."
            : "Communicate with clients, negotiate milestones, and coordinate project delivery."}
        </p>
      </div>

      <div className="flex-1 bg-surface border border-surface-border rounded-2xl overflow-hidden flex min-h-0 shadow-lg">

        {/* Sidebar */}
        <div className="w-1/3 border-r border-surface-border flex flex-col min-h-0 bg-surface/50">
